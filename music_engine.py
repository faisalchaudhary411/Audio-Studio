...
def _init_db():
    with _db_lock:
        ...
        for col, typedef in (("license_key", "TEXT"), ("quota_billed", "INTEGER DEFAULT 0")):
            try:
                conn.execute(f"ALTER TABLE music_jobs ADD COLUMN {col} {typedef}")
            except Exception:
                pass
        conn.commit()
        conn.close()
...
def _insert_job(job_id: str, license_key: str = "") -> None:
    ...
    with _db_lock:
        ...
        try:
            conn.execute(
                "INSERT INTO music_jobs (job_id, status, created_at, updated_at, license_key, quota_billed) "
                "VALUES (?, 'queued', ?, ?, ?, 0)",
    ...
    with _db_lock:
        ...
        try:
            ...
            row = conn.execute(
                ...
                "FROM music_jobs WHERE job_id = ? AND status = 'done'",
                (job_id,),
            ).fetchone()
            ...
            if not key:
                ...
                return ""
            conn.execute(
                "UPDATE music_jobs SET quota_billed = 1, updated_at = ? WHERE job_id = ?",
                (time.time(), job_id),
            )
            conn.commit()
            return key
